...
const SelectItem = React.forwardRef(({ className, children, value, ...props }, ref) => {
    // Radix Select forbids empty-string values for items.
    // If any page accidentally renders value="", it would crash the whole app.
    // We defensively map it to a sentinel and emit a clear console error.
    const safeValue = value === "" || value == null ? "__empty__" : value;
    if (value === "" || value == null) {
        console.error("SelectItem recebeu value vazio. O Radix Select não permite value=\"\" em itens; use um valor sentinela (ex.: 'all') e faça o mapeamento no estado.", { originalValue: value });
    }
    return (_jsxs(SelectPrimitive.Item, { ref: ref, value: safeValue, className: cn("relative flex w-full cursor-default select-none items-center rounded-sm py-1.5 pl-8 pr-2 text-sm outline-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50 focus:bg-accent focus:text-accent-foreground", className), ...props, children: [_jsx("span", { className: "absolute left-2 flex h-3.5 w-3.5 items-center justify-center", children: _jsx(SelectPrimitive.ItemIndicator, { children: _jsx(Check, { className: "h-4 w-4" }) }) }), _jsx(SelectPrimitive.ItemText, { children: children })] }));
});

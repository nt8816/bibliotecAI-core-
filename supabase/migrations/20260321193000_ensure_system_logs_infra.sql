...
BEGIN
  ...
  IF v_escola_id IS NULL AND to_regprocedure('public.get_user_escola_id(uuid)') IS NOT NULL THEN
    EXECUTE 'SELECT public.get_user_escola_id($1)' INTO v_escola_id USING auth.uid();
  END IF;
